...
open class Action<SubjectType, ParametersType, ResultType> : AnyAction {
	...
	deinit {
		ActionConfig.logger?.debug("Deiniting an action of type \(type(of: self))")
	}
	...
	be called w/ an `ActionError.actionIsRunning` error.
		...
		let (shouldLaunch, handlerCallArgument) = stateSyncQueue.sync{ () -> (Bool, Result<ResultType, Error>?) in
			guard !currentState.isRunning else {
				...
				if shouldJoinRunningAction(self.latestParameters!) {
					...
					else               {ActionConfig.logger?.warning("Asked to join a running action of type \(type(of: self)) for subject \(self.subject) but no handler given…")}
					return (false, nil)
				} else {
					return (false, .failure(ActionError.actionIsRunning))
				}
			}
	...
	public final func weaken() throws {
		try stateSyncQueue.sync{
			...
			case .running:
				throw ActionError.actionIsRunning
			}
		}
	}
	...
	public final func clearLatestParameters() throws {
		try stateSyncQueue.sync{
			guard !currentState.isRunning else {throw ActionError.actionIsRunning}
